...
# ╔═╡ 8bfa3b0f-4438-4551-9a8f-49d519d2595d
using Plots

# ╔═╡ fa837e87-be08-4e08-a749-f73566e327cb
lateral_acceleration = -3.:0.1:3.
...
@bind aego Slider(-3.:0.1:3., default=0., show_value=true)

# ╔═╡ 71b59ab9-f3e7-40a1-8446-b23a41703aea
inputs = Float32[
	lateral_acceleration';
	ones(3, n) .* [roll_compensation, vego, aego]
]
...
model_weights = let
    weights = all_weights[car]
    Dict{String, Array{Float32}}([w => hcat(weights[w]...) for w in keys(weights)])
end

# ╔═╡ 40501ecf-3f0a-4350-b15c-50a313291ada
struct NanoFFModel{T}
	norm_in::Matrix{T}
	w₁::Matrix{T}
	w₂::Matrix{T}
	w₃::Matrix{T}
	...
	function NanoFFModel{T}(weights::Dict{String, Array{T}}) where T
		new(
			weights["input_norm_mat"]',
			weights["w_1"],
			weights["w_2"],
			weights["w_3"],
			weights["w_4"],
...
function (m::NanoFFModel{T})(x::VecOrMat{T}) where T
	x = (x .- m.norm_in[:, 1]) ./ (m.norm_in[:, 2] - m.norm_in[:, 1])
	x = relu.(m.w₁*x .+ m.b₁)
	x = relu.(m.w₂*x .+ m.b₂)
	x = relu.(m.w₃*x .+ m.b₃)
	x = m.w₄*x .+ m.b₄
